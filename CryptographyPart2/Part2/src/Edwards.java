...
public class Edwards {
    ...
    public Point getPoint(BigInteger y, Boolean x_hasLSB) {
        ...
        Point __p2 = pnt.add(pnt);
        Point __p4 = __p2.add(__p2);

        if (__p4.isZero()) {
            return new Point(); // reject small-order (1,2,4)
        }
    ...
    /**
     * Human-readable curve description, as requested by the Part-2 spec.
     * Example: "NUMS ed-256-mers*: x^2 + y^2 = 1 + d*x^2*y^2 (mod p)"
     */
    @Override
    public String toString() {
